...
class RawEditor extends StatefulWidget {
  const RawEditor({
    required this.onCustomMenuItemSelected,
  ...
  static Widget defaultContextMenuBuilder(BuildContext context, RawEditorState state) {
...
class RawEditorState extends EditorState with AutomaticKeepAliveClientMixin<RawEditor>, WidgetsBindingObserver, TickerProviderStateMixin<RawEditor>, RawEditorStateTextInputClientMixin, RawEditorStateSelectionDelegateMixin {
  ...
  KeyEventResult _handleTabKey(RawKeyEvent event) {
    ...
    if (controller.selection.baseOffset != controller.selection.extentOffset) {
      ...
      if (parentBlock.style.containsKey(Attribute.ol.key) || parentBlock.style.containsKey(Attribute.ul.key) || parentBlock.style.containsKey(Attribute.checked.key)) {
  ...
  void _moveCursor(int chars) {
    ...
    controller.updateSelection(controller.selection.copyWith(baseOffset: selection.baseOffset + chars, extentOffset: selection.baseOffset + chars), ChangeSource.LOCAL);
  }
  ...
  EditableTextLine _getEditableTextLineFromNode(Line node, BuildContext context) {
    ...
    final editableTextLine = EditableTextLine(node, null, textLine, 0, _getVerticalSpacingForLine(node, _styles), _textDirection, controller.selection, widget.selectionColor, widget.enableInteractiveSelection, _hasFocus, View.of(context).devicePixelRatio, _cursorCont);
...
class _UpdateTextSelectionToAdjacentLineAction<T extends DirectionalCaretMovementIntent> extends ContextAction<T> {
  ...
  void stopCurrentVerticalRunIfSelectionChanges() {
    ...
    final continueCurrentRun = currentSelection.isValid && currentSelection.isCollapsed && currentSelection.baseOffset == runSelection.baseOffset && currentSelection.extentOffset == runSelection.extentOffset;
...
///    [EditableText]
typedef QuillEditorContextMenuBuilder = Widget Function(BuildContext context, RawEditorState rawEditorState);
